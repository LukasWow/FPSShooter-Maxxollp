...
public class PlayerGrapplingHook : MonoBehaviour
{
    ...
    private void Update()
    {
        if (crossHairSpinningPart != null)
        {
            ...
            if (Physics.Raycast(CameraMovement.cam.transform.position, CameraMovement.cam.transform.forward, out hitInfo, maxGrappleDistance, layerMask))
            {
                hitName = hitInfo.collider.name;
                if (Physics.Raycast(CameraMovement.cam.transform.position, CameraMovement.cam.transform.forward, out hitInfo, maxGrappleDistance))
                {
