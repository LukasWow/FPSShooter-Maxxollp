...
public class SFPSC_WallRun : MonoBehaviour
{
    ...
    {
        wallRunning = false;
        CameraMovement.cam.rotZ = 0.0f; // Resetting the z rotation on the camera (if the player shoots and the camera shakes it won't be such a problem)
        blocked = true;
        ...
        bool right) // if right is false it means that the wall is on the left side
    {
        ...
        CameraMovement.cam.rotZ = right ? cameraTiltAngle : -cameraTiltAngle;
    }
